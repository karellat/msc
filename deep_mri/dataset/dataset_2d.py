...
def _process_path(file_path, img_size):
...
def _generator(file_list, img_size):
    for file_name in file_list:
        img, label = _process_path(file_name, img_size)
        yield img, label


def factory(train_files, valid_files, img_size=193, shuffle=True, seed=42):
    ...
    train_ds = tf.data.Dataset.from_generator(_generator,
                                              ...
                                              args=[train_files, img_size])
    valid_ds = tf.data.Dataset.from_generator(_generator,
                                              output_types=(tf.float32, tf.bool),
                                              output_shapes=(img_shape, (3,)),
                                              args=[valid_files, img_size])
