...
def _process_path(file_path, img_size, channels, class_names):
...
def _generator(file_list, img_size, channels, class_names):
    for file_name in file_list:
        img, label = _process_path(file_name, img_size, channels, class_names)
        yield img, label


def factory(train_files, valid_files, dropping_group=None, img_size=193, channels=3, shuffle=True, seed=42):
    ...
    train_ds = tf.data.Dataset.from_generator(_generator,
                                              ...
                                              args=[train_files, img_size, channels, class_names])
