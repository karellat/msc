...
subject_model = tf.keras.models.load_model("/home/karelto1/master_thesis/logs/final_3d/hosseini2_subjects/models/")

config['shuffle_strategy'] = 'subjects'
config['log_name'] += '_subjects'


subject_model_3 = config_to_model(config)


for i in range(1, 25):
    subject_model_3.layers[i].set_weights(subject_model.layers[i].get_weights())
...
subject_model_3.fit(train_ds.batch(batch_size),
          epochs=epochs,
