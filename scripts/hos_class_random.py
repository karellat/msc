...
random_model = tf.keras.models.load_model("/home/karelto1/master_thesis/logs/final_3d/hosseini3_random/models/")

config['shuffle_strategy'] = 'random'
config['log_name'] += '_random'


random_model_class = config_to_model(config)


for i in range(1, 25):
    random_model_class.layers[i].set_weights(random_model.layers[i].get_weights())
...
random_model_class.fit(train_ds.batch(batch_size),
          epochs=epochs,
